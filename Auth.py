...
def generate_keypair(ecc_setup, s=None):
    ...
    if s is None:
        s = 8022734443851026088049500214544140079735552021820380428396264672545508513883;
...
def sign(priv, msg, u=None):
    ...
    while True:
        ...
        if u is None:
            u = 100148887987684266425327374973665695692770751634129884762123621529589463293098;
        ...
        if c == 0:
            print(f"c={c}")
            continue
        d = (modinv(u, r) * (hash1 + priv.secret * c)) % r
        if d == 0:
            print(f"d={d}")
            continue
        break
...
def main():
        ...
        myMQTTClient = AWSIoTMQTTClient("ECdsaClientID") #random key, if another connection using the same key is opened the previous one is auto closed by AWS IOT
        myMQTTClient.configureEndpoint("a3lznbxcd3vz1d-ats.iot.us-west-2.amazonaws.com", 8883)

        myMQTTClient.configureCredentials("/home/pi/AWSIot/AmazonRootCA1.pem", "/home/pi/AWSIot/privatekey.pem.key", "/home/pi/AWSIot/certificate.pem.crt")
        ...
        print("Publishing Data from RPi");
        ...
        signature = sign(priv, hash1)
        #print(f"signature (c,d) = {signature.c}, {signature.d}")
 
        #validation = verify_signature(pub, hash1, signature)
        #print(f"verify_signature(pub, {hash1}, signature) = {validation}")
       
        myMQTTClient.publish(
            topic="topic_1",
            QoS=1,
            payload="Signature : {},{}\nPublic Key : {},{}\n Corresponding Hash : {}".format(signature.c,signature.d,pub.W.x,pub.W.y,hash1))
        print();
        print();
